...
/**
 * Outlook-safe HTML for Cancellation Request Approved emails.
 *
 * Expected keys (HTML-escaped where needed):
 * - requester_surname, employee_name, location_name, group_abbr
 * - cr_ref, req_ref
 * - dispatch_dates, reason
 * - original_dispatch_status (e.g. Cancelled)
 * - kdt_cta_url, khi_cta_url, logo_url
 * - test_mode_placeholder
 */
function buildCancellationRequestApprovedEmailHtml(array $data): string
{
    ...
    $crRef = (string)($data['cr_ref'] ?? '');
    $reqRef = (string)($data['req_ref'] ?? '');
    $dispatchDates = (string)($data['dispatch_dates'] ?? '');
    $reason = (string)($data['reason'] ?? '');
    $originalDispatchStatus = (string)($data['original_dispatch_status'] ?? 'Cancelled');
    ...
    $dispatchDates = $dispatchDates !== '' ? $dispatchDates : $dash;
    $reason = $reason !== '' ? $reason : $dash;
    $originalDispatchStatus = $originalDispatchStatus !== '' ? $originalDispatchStatus : 'Cancelled';

    $crBadge = $crRef !== ''
        ? '<span style="display:inline-block;padding:4px 10px;background-color:#F6339A;color:#000000;border-radius:6px;font-size:12px;font-weight:700;line-height:1.3;">'
            . $crRef
            . '</span>'
                ...
                <tr>
                    <td style="padding:30px 34px 26px 34px;">
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;width:100%;">
                            <tr>
                                <td valign="top" style="font-size:32px;line-height:34px;font-weight:700;color:#000000;padding:0;">
                                    Cancellation Request<br>Approved
                                </td>
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;width:100%;margin-top:36px;">
                            ...
                            <tr>
                                <td style="font-size:13px;line-height:20px;color:#000000;padding:0 0 14px 0;">Your cancellation request has been approved. The original dispatch request status is <strong style="font-weight:700;color:#000000;">' . $originalDispatchStatus . '.</strong></td>
                            </tr>
                            <tr>
                                <td style="font-size:13px;line-height:20px;color:#000000;padding:0;">You can find this dispatch request in the &ldquo;Cancelled&rdquo; tab of the request list.</td>
                            </tr>
                        </table>
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;margin-top:28px;background-color:#F9F9F9;border:1px solid #E9E9E9;border-radius:14px;border-collapse:separate;">
                            ...
                            <tr>
                                ...
                                <td width="50%" valign="top" style="width:50%;padding:14px 16px 16px 14px;">
                                    ...
                                    <div style="font-size:13px;line-height:18px;font-weight:700;color:#000000;word-break:break-word;">' . $dispatchDates . '</div>
                                </td>
                            </tr>
                        </table>

                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;margin-top:18px;background-color:#F9F9F9;border:1px solid #E9E9E9;border-radius:12px;border-collapse:separate;">
                            <tr>
                                <td style="padding:14px 16px;">
                                    <div style="font-size:10px;line-height:12px;letter-spacing:0.04em;text-transform:uppercase;color:#959595;margin:0 0 8px 0;">REASON FOR CANCELLATION</div>
                                    <div style="font-size:13px;line-height:20px;font-weight:700;color:#000000;word-break:break-word;">' . $reason . '</div>
                                </td>
                            </tr>
                        </table>
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:separate;width:100%;margin-top:18px;background-color:#F2FFF7;border:1px solid #E9E9E9;border-left:3px solid #2EAD5C;border-radius:10px;">
                            <tr>
                                <td width="36" valign="middle" style="width:36px;padding:14px 0 14px 14px;vertical-align:middle;">
                                    <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
                                        <tr>
                                            <td align="center" valign="middle" style="width:18px;height:18px;border:1px solid #2EAD5C;border-radius:50%;color:#2EAD5C;font-size:11px;line-height:18px;font-weight:700;">i</td>
                                        </tr>
                                    </table>
                                </td>
                                <td style="padding:14px 16px 14px 8px;font-size:13px;line-height:20px;color:#000000;">
                                    <div style="font-size:13px;line-height:20px;font-weight:700;color:#000000;margin:0 0 12px 0;">Original dispatch request status: <span style="color:#2EAD5C;">' . $originalDispatchStatus . '</span></div>
                                    <div style="font-size:12px;line-height:18px;font-weight:400;color:#000000;">The cancellation request was approved. The original dispatch request has been cancelled, and the scheduled dispatch will no longer proceed.</div>
                                </td>
                            </tr>
                        </table>
                                                ...
                                                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;width:100%;">
                                                    <tr>
                                                        <td align="center" bgcolor="#000000" style="background-color:#000000;border-radius:8px;">
                                                            <a href="' . $kdtCtaUrl . '" style="display:block;padding:12px 10px;font-family:Arial, Helvetica, sans-serif;font-size:12px;line-height:16px;font-weight:700;color:#FFFFFF;text-decoration:none;text-align:center;border-radius:8px;">View Cancellation Request&nbsp;&nbsp;&rarr;</a>
                                                        </td>
                                                    </tr>
                                                </table>
                                            </td>
...
/**
 * Outlook-safe HTML for Cancellation Request Declined emails.
 * Same family layout as buildCancellationRequestApprovedEmailHtml(); declined/info styling differs.
 *
 * Expected keys (HTML-escaped where needed):
 * - requester_surname, employee_name, location_name, group_abbr
 * - cr_ref, req_ref
 ...
 * - test_mode_placeholder
 */
function buildCancellationRequestDeclinedEmailHtml(array $data): string
{
    $requesterSurname = (string)($data['requester_surname'] ?? '');
    $employeeName = (string)($data['employee_name'] ?? '');
    $locationName = (string)($data['location_name'] ?? '');
                ...
                <tr>
                    <td style="padding:30px 34px 26px 34px;">
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;width:100%;">
                            <tr>
                                <td valign="top" style="font-size:32px;line-height:34px;font-weight:700;color:#000000;padding:0;">
                                    Cancellation Request<br>Declined
                                </td>
                                <td align="right" valign="top" style="padding:4px 0 0 12px;white-space:nowrap;">
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;width:100%;margin-top:36px;">
                            ...
                            <tr>
                                <td style="font-size:13px;line-height:20px;color:#000000;padding:0 0 14px 0;">Your cancellation request has been reviewed by the KDT President.</td>
                            </tr>
                            <tr>
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:separate;width:100%;margin-top:18px;background-color:#F2F8FF;border:1px solid #E9E9E9;border-left:3px solid #4A85DE;border-radius:10px;">
                            <tr>
                                <td width="36" valign="middle" style="width:36px;padding:14px 0 14px 14px;vertical-align:middle;">
                                    <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
                                        <tr>
                                            <td align="center" valign="middle" style="width:18px;height:18px;border:1px solid #2E74AD;border-radius:50%;color:#2E74AD;font-size:11px;line-height:18px;font-weight:700;">i</td>
                                        </tr>
                                    </table>
                                </td>
                                <td style="padding:14px 16px 14px 8px;font-size:13px;line-height:20px;color:#000000;">
                                    <div style="font-size:13px;line-height:20px;font-weight:700;color:#000000;margin:0 0 12px 0;">Dispatch schedule remains unchanged</div>
                                    <div style="font-size:12px;line-height:18px;font-weight:400;color:#000000;">The cancellation request was declined. The original dispatch request and its approved schedule remain unchanged.</div>
                                </td>
                            </tr>
                        </table>
